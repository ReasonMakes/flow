...
namespace Photon.Pun.Demo.Asteroids
{
    public class AsteroidsGameManager : MonoBehaviourPunCallbacks
    {
        ...
        private IEnumerator EndOfGame(string winner, int score)
        {
            ...
            while (timer > 0.0f)
            {
                InfoText.text = string.Format("Player {0} won with {1} points.\n\n\nLeaving game in {2} seconds.", winner, score, timer.ToString("F0"));
        ...
        private void StartGame()
        {
            ...
            //Vary spawn point so clients don't spawn inside each other
            float startAngle = (360.0f / PhotonNetwork.CurrentRoom.PlayerCount) * PhotonNetwork.LocalPlayer.GetPlayerNumber();
            float x = 20.0f * Mathf.Sin(startAngle * Mathf.Deg2Rad);
            float z = 20.0f * Mathf.Cos(startAngle * Mathf.Deg2Rad);
            Vector3 position = new Vector3(x, 2.0f, z);
            Quaternion rotation = Quaternion.Euler(0.0f, startAngle, 0.0f);

            //Instantiate local player on the network
            GameObject instanceSpaceShip = PhotonNetwork.Instantiate("Spaceship", position, rotation, 0);      // avoid this call on rejoin (ship was network instantiated before)

            //Pass game manager script
            instanceSpaceShip.GetComponent<Spaceship>().gameManager = this;
            instanceSpaceShip.GetComponent<Spaceship>().startAngle = startAngle;
        ...
        private bool CheckAllPlayerLoadedLevel()
        {
            foreach (Player p in PhotonNetwork.PlayerList)
            {
                ...
                if (p.CustomProperties.TryGetValue(AsteroidsGame.PLAYER_LOADED_LEVEL, out object playerLoadedLevel))
                {
        ...
        private void CheckEndOfGame()
        {
            ...
            foreach (Player p in PhotonNetwork.PlayerList)
            {
                if (p.CustomProperties.TryGetValue(AsteroidsGame.PLAYER_LIVES, out object lives))
                {
                    if ((int)lives > 0)
                    {
                        allDestroyed = false;
