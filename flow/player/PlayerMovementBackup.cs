...
public partial class PlayerMovementBackup : RigidBody3D
{
    //Diagnostics
    ...
    private bool IsWallrunning = false; //wallrunning overrides climbing
    ...
    private const float ThrustWallrunCoefficient = 1f; //25f; //applied after jerk is added
    private const float ThrustClimbCoefficient = 1f; //0.1f; //applied after jerk is added

    private const float SlopeMovementMaxSpeed = 10f; //applied the same as aligned twitch running

    private float ThrustTiredWishingIntoSlopeCoefficient = 1f; //this is dynamically modified when tired-climbing
    ...
    public override void _PhysicsProcess(double deltaDouble)
    {
        ...
        else if (IsClimbing || IsWallrunning)
        {
    ...
    private float GetDrag()
    {
        ...
        if (OnFlat || OnSlope) //&& JumpCooldown < JumpCooldownPeriod - JumpNoDragPeriod)
        {
            ...
            if (OnFlat)
            {
                ...
                //no else because default drag is for this condition (standing on flat)
            }
            else
            {
                //Slope
    ...
    private Vector3 GetVectorAlignedJerked(float delta, Vector3 wishDirection, float jerkCoefficient)
    {
        ...
        //COMBINE
        float runMagnitude = ((runDynamicAccelerationTwitch * runAlignmentScaled) + jerk) * (Thrust * ThrustTiredWishingIntoSlopeCoefficient * slopeCoefficient);
    ...
    private Vector3 GetDirection(Vector3 wishDirectionRaw, PhysicsDirectBodyState3D state)
    {
        ...
        //Prevent moving directly into a surface from slowing down movement speed
        ...
        Vector3 thrustDirection = wishDirection;//(wishDirection - (surfaceNormalWishingInto * wishDirection.Dot(surfaceNormalWishingInto))).Normalized();
        ...
        bool isWishingToThrustUp = thrustDirection.Dot(Vector3.Up) >= SlopeDotUp; //isWishingToClimb as long as this is into a surface
        //Stick to wall
